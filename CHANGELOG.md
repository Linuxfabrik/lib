...
* container.py: new module for the answers every consumer of a container engine has to give the same way. `get_engine_error()` tells a refused permission, which says nothing about the engine and is UNKNOWN, apart from an engine that is down or gone, `strip_daemon_error()` reduces what an engine answered to the sentence somebody can act on, `strip_task_id()` takes the task id a swarm appends to a container name off again, which otherwise makes every rescheduling look like a new container, and `run()` gives every client call of a run what is left of one time budget and reports an engine that does not answer in time as a warning
* db_sqlite.py: `connect(in_memory=True)` opens a database private to the process, `cut_per_sensor()` trims a history table per sensor instead of by total row count, `first_seen()` records when each item of an observed set turned up, so a consumer can hold an alert back until an item has persisted, and `forget_sensors()` deletes the history of the sensors that no longer exist
* disk.py: `under_root()` relocates a path below a root, whatever it is anchored on, and drops the segments that would walk back out of it. `is_symlink()` tests for a symbolic link, and `read_file()` and `read_env()` take `allowed_roots` and `nofollow` to confine a read to a trusted root
...
* redfish.py: `get_expand_suffix()` asks for a single `$expand` level, which already inlines every member of a collection, instead of the deepest one a controller offers, which made its answers many times larger. `get_auth_header()` restarts the lifetime of a cached session token each time it hands the token out, so a controller sees one login instead of one per `SessionTimeout` ([#350](https://github.com/Linuxfabrik/lib/issues/350))
