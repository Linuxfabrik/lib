...
* lftest.py: `run_mariadb_from_containerfile(containerfile_path, ...)` builds a per-plugin Containerfile via testcontainers' DockerImage and yields `(container, defaults_file)` like `run_mariadb()`. Plugins move their test-image matrix into `unit-test/containerfiles/<name>` so each plugin owns its supported MariaDB / MySQL LTS coverage instead of relying on a hardcoded image list in the lib
