...
def parse_atom(soup):
    ...
    result['updated_parsed'] = base2.timestr2datetime(result['updated'][0:19], pattern='%Y-%m-%dT%H:%M:%S')
    ...
    for entry in soup.find_all('entry'):
        ...
        # cut the timezone part
        tmp['updated_parsed'] = base2.timestr2datetime(tmp['updated'][0:19], pattern='%Y-%m-%dT%H:%M:%S')
        try:
...
def parse_rss(soup):
    ...
    # cut the timezone part
    result['updated_parsed'] = base2.timestr2datetime(result['updated'][0:25], pattern='%a, %d %b %Y %H:%M:%S')
    ...
    for entry in soup.find_all('item'):
        ...
        # cut the timezone part
        tmp['updated_parsed'] = base2.timestr2datetime(tmp['updated'][0:25], pattern='%a, %d %b %Y %H:%M:%S')
        try:
...
def parse(feed_url, insecure=False, no_proxy=False, timeout=5, encoding='urlencode'):
    ...
    success, xml = url2.fetch(feed_url, insecure=insecure, no_proxy=no_proxy, timeout=timeout,
        encoding='urlencode')
    if not success:
        return (False, xml)
