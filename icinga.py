...
# take care of Icinga and throttle the amount of requests, don't overload it with too fast subsequent api-calls
DEFAULT_SLEEP = 1.0


def api_post(url, username, password, data={}, method_override='', insecure=True, no_proxy=False, timeout=3):
    ...
    >>> }
    >>> result = lib.base.coe(lib.icinga.api_post(url, args.USERNAME, args.PASSWORD, data=data, method_override='GET', timeout=3))
    """
    ...
    result = lib.url.fetch_json(url, insecure=insecure, no_proxy=no_proxy, timeout=timeout, header=header, data=data, encoding='serialized-json')
    time.sleep(DEFAULT_SLEEP)
    return result


def get_service(url, username, password, servicename, attrs='state'):
    """POST a high level request to the Icinga `objects/service` API (with less possibilities). The service name should be unique and has to be taken from the `__name` attribute.

    >>> url = 'https://icinga-server:5665'
    >>> result = lib.base.coe(lib.icinga.get_service(url, args.USERNAME, args.PASSWORD, servicename='hostname!special-service', attrs='state,acknowledgement'))
    """
...
def set_ack(url, username, password, objectname, type='service', author='Linuxfabrik lib.icinga'):
    """Allows you to acknowledge the current problem for hosts or services. By acknowledging the current problem, future notifications (for the same state if sticky is set to false) are disabled.  The host or service name should be unique and has to be taken from the `__name` attribute.

    Acknowledging an already acknowledged problem is ok, while acknowleding a host or service in OK state leads to a *500 internal server error".
    """
    ...
    return api_post(url=url, username=username, password=password, data=data, insecure=True)
...
def set_downtime(url, username, password, objectname, type='service', starttime=int(time.time()), endtime=int(time.time())+60*60, author='Linuxfabrik lib.icinga'):
    ...
    success, result = api_post(url=url, username=username, password=password, data=data, insecure=True)
