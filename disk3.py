...
def grep_file(filename, pattern):
    ...
    >>> success, nc_version=lib3.disk.grep_file('version.php', r'\\$OC_version=array\\((.*)\\)')
...
def walk_directory(path, exclude_pattern=r'', include_pattern=r'', relative=True):
    ...
    >>> lib3.disk.walk_directory('/tmp')
    ['cpu-usage.db', 'segv_output.MCiVt9']
    >>> lib3.disk.walk_directory('/tmp', exclude_pattern='.*Temp-.*', relative=False)
